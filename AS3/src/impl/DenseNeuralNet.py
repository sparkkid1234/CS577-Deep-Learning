...
class DNN():
    def __init__(self, input_dim, output_dim, depth, units,dropout=False,batch_norm=False,l2=False):
    ...
    def build_model(self,optimizer='adam',hidden_act = 'relu',out_act='softmax',learning_rate=0.000001,loss = "categorical_crossentropy",metrics = ['acc']):
        self.model = models.Sequential()
        if self.l2:
            self.model.add(Dense(self.units,kernel_regularizer=regularizers.l2(0.01), input_shape=(self.input_dim,)))
        else:
            self.model.add(Dense(self.units, input_shape=(self.input_dim,)))
        if self.batch_norm:
        ...
        for i in range(self.depth-1):
            if self.l2:
                self.model.add(Dense(self.units, activation = hidden_act,kernel_regularizer=regularizers.l2(0.01)))
            else:
                self.model.add(Dense(self.units, activation = hidden_act))
            if self.dropout:
                self.model.add(layers.Dropout(0.2))
